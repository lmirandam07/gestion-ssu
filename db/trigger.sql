...
AFTER
UPDATE ON propuesta_proyecto FOR EACH ROW BEGIN IF NEW.id_estado = 1 THEN
...
AFTER
INSERT ON proyecto_usuario FOR EACH ROW BEGIN
UPDATE proyecto
SET participantes_pro = (participantes_pro - 1)
...
SET @horas_proyecto = (
        SELECT FORMAT(
                TIME_TO_SEC(TIMEDIFF(hora_final_pro, hora_inicio_pro)) / 3600,
                0
            ) AS diferencia
        FROM proyecto
        WHERE id_proyecto = NEW.id_proyecto
...
UPDATE usuario
SET total_horas = (total_horas + @horas_proyecto)
